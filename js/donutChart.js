...
DonutChart.prototype.initVis = function(){
  ...
  vis.pie = d3.pie()
    .padAngle(0.03)
    .value(d => { return d.data[vis.variable]; })
    .sort(null);
  ...
  vis.g = vis.svg.append('g')
    ...
    // s.attr('transform', `translate(${vis.margin.left} + ${vis.width}/2, ${vis.margin.top} + ${vis.height}/2)`);
...
DonutChart.prototype.updateVis = function(){
  ...
  vis.path.exit()
    .datum( (d, i) => {return findNeighborArc(i, vis.data1, vis.data0, key) || d; })
    .transition()
    .duration(750)
  ...
  vis.path.transition()
    .duration(750)
    .attrTween('d', arcTween)
    .attr('fill-opacity', d => { return (d.data.coin == vis.activeCoin) ? 1 : 0.3; })

  vis.path.enter()
    .append('path')
    .each((d, i) => { this._current = findNeighborArc(i, vis.data0, vis.data1, key) || d; })
    .attr("fill", d => {  return color(d.data.coin) })
    .attr("fill-opacity", d => {
